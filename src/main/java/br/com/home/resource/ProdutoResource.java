...
public class ProdutoResource {
	...
	public Response atualizar(Produto produto) {
		try {
			return Response.ok(service.atualizar(produto)).status(Status.OK).build();
		} catch (Exception e) {
	...
	@Path("/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response buscarPorId(@PathParam("id") Integer idProduto) {
		try {
	...
	public Response salvar(Produto produto) {
		try {
			return Response.ok(service.salvar(produto)).status(Status.CREATED).build();
		} catch (Exception e) {
			return Response.ok(new MensagemResposta(e.getMessage())).status(Status.BAD_REQUEST).build();
		}
	}
